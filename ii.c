...
PHP_MINIT_FUNCTION(ii)
{
	...
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_ISAM",		II_STRUCTURE_ISAM,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_CISAM",	II_STRUCTURE_CISAM,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_BTREE",	II_STRUCTURE_BTREE,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_CTREE",	II_STRUCTURE_CBTREE,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_HASH",		II_STRUCTURE_HASH,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_CHASH",	II_STRUCTURE_CHASH,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_HEAP",		II_STRUCTURE_HEAP,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("INGRES_STRUCTURE_CHEAP",	II_STRUCTURE_CHEAP,		CONST_CS | CONST_PERSISTENT);
...
PHP_FUNCTION(ingres_query)
{
	...
	{ /* convert ? to ~V so we don't have to prepare the query */
		tmp_statement=emalloc(Z_STRLEN_PP(query) + ( ii_link->paramCount*3 ) + 1);
		memcpy (tmp_statement,  Z_STRVAL_PP(query), Z_STRLEN_PP(query));
...
static char *php_ii_convert_param_markers (char *statement TSRMLS_DC)
{
	...
	new_statement = emalloc ( statement_length + (parameter_count*3) + 1); /* allow for space either side and a null*/
	sprintf(new_statement,"\0");
			...
			if ( ( paramtypes != NULL ) && ( Z_TYPE_PP(paramtypes) == IS_STRING ))
			{
				...
				switch (types[param - with_procedure])
				{
					case 'B': /* long byte */
						...
						setDescrParm.sd_descriptor[param].ds_precision = 0;
						setDescrParm.sd_descriptor[param].ds_scale = 0;
						setDescrParm.sd_descriptor[param].ds_columnType = IIAPI_COL_QPARM;
						if ( ii_link->procname == NULL )
						{
							setDescrParm.sd_descriptor[param].ds_columnName = NULL;
						}
